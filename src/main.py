...
def process_sequences(sequences_dict: dict[str, str], source_name: str, min_length: int, start_codons: set[str], results_dir: Path) -> str:
    """
...
def process_single_file(file_path: Path, min_length: int, start_codons: set[str], results_dir: Path) -> str:
    ...
    try:
        ...
        return process_sequences(sequences_dict, file_path.stem, min_length, start_codons, results_dir)
    except Exception as e:
        return f"Error reading {file_path.name}: {str(e)}"

def process_ncbi_id(ncbi_id: str, min_length: int, start_codons: set[str], results_dir: Path) -> str:
    """
    ...
    try:
        ...
        return process_sequences(sequences_dict, ncbi_id, min_length, start_codons, results_dir)
    except Exception as e:
...
def main() -> None:
    ...
    parser.add_argument("--ncbi", type=str, default=None, help="Comma-separated list of NCBI accession IDs to fetch and analyze")
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for task_type, task_val in tasks:
            if task_type == 'file':
                futures.append(executor.submit(process_single_file, task_val, args.min_length, start_codons_set, results_dir))
            else:
                futures.append(executor.submit(process_ncbi_id, task_val, args.min_length, start_codons_set, results_dir))

        completed: int = 0
        for future in as_completed(futures):
            results.append(future.result())
            completed += 1
            print_progress_bar(completed, len(tasks), prefix='Progress:', suffix='Complete', length=40)
